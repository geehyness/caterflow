    ...
    const addSupplierToStockItem = async (itemId: string, supplierId: string) => {
        try {
            const resp = await fetch('/api/procurement/suppliers', {
                ...
                body: 
JSON.stringify({ itemId, supplierId }),
            });
if (!resp.ok) {
...
// Update local state to reflect the change
            setStockItems(prev => {
                ...
                if (!newSuppliersList.find(x => x._id 
=== supplierId)) {
...
setStockItems(prev => {
                ...
                return {
                    ...prev,
                    [itemId]: { ...s, 
primarySupplier: { _id: supplierId, name: suppliers.find(x => x._id === supplierId)?.name || '' } }
                };
            });
return json;
        } catch (err) {
...
return selectedPO.orderedItems.every(item => {
    ...
    const getRowError = (item: any) => {
        const supplier = editedSuppliers[item._key] ??
item.supplier?._id;
        const totalPrice = editedPrices[item._key];
